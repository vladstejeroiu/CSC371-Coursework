...
void Areas::setArea(std::string localAuthorityCode){

  this->areas_container[localAuthorityCode] = new Area(localAuthorityCode);
...
Area *Areas::getArea(std::string localAuthorityCode){
  ...
  if(areas_container.count(localAuthorityCode) == 0) {
    throw std::out_of_range("No area with select authority Code");
  }

  return areas_container[localAuthorityCode];
}
...
void Areas::populateFromAuthorityCodeCSV(
    ...
    while(getline(is, str_line, '\n')){
      ...
      while ((pos_end = str_line.find (delimiter, pos_start)) != std::string::npos) {
          token = str_line.substr (pos_start, pos_end - pos_start);
          pos_start = pos_end + delim_len;
          res.push_back (token);
      }

      res.push_back(str_line.substr(pos_start));
      ...
      if(cols.size() != res.size()) {
        std::cout << "Invalid size error..." << res.size() << " : " << cols.size() << "\n";
      }

      setArea(res[0]);
      areas_container[res[0]]->setName("eng", res[1]);
      areas_container[res[0]]->setName("cym", res[2]);
  }
...
void Areas::populateFromAuthorityByYearCSV(
    ...
    while(getline(is, str_line, '\n')){
      ...
      if(firstLine) {
        ...
        res.erase(res.begin());
        for(auto e : res) {
          years.push_back(atoi(e.c_str()));
        }
        firstLine = false;
        continue;
      }

      if(years.size() + 1 != res.size()) {
        std::cout << "Invalid size error..." << res.size() << " : " << cols.size() << "\n";
      }

      Measure measure("Measure name", "Measure label");

      std::string code = res[0];
      for(unsigned int i=1;i<res.size(); i++) {
        measure.setValue(years[i-1], std::stod(res[i].c_str()));
      }

      areas_container[res[0]]->setMeasure("Measure", measure);
  }
...
std::string Areas::toJSON() const {
  ...
  for(auto &apair : areas_container) {
    j[apair.first] = apair.second->toJSON();
  }
