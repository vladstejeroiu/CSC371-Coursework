...
int BethYw::run(int argc, char *argv[]) {
  ...
  std::string dir = "datasets" + DIR_SEP;
  ...
  std::cout << datasetsToImport.size() << "\n";
...
std::vector<BethYw::InputFileSource> BethYw::parseDatasetsArg(
  ...
  if(args.count("datasets") == 0) {
    std::vector<BethYw::InputFileSource> all;
    for(unsigned int i =0; i<numDatasets; i++) {
      all.push_back(allDatasets[i]);
    }
    return all;
  }
  ...
  for(unsigned int i = 0; i < numDatasets; i++) { 
    for(auto s : inputDatasets) {
      if(s == allDatasets[i].CODE) {
...
std::unordered_set<std::string> BethYw::parseAreasArg(
  ...
  if(args["areas"].count() == 0 || (temp.size() == 1 && temp[0] == "all")) {
    return areas;
  }
...
std::unordered_set<std::string> BethYw::parseMeasuresArg(
  ...
  if(args["measures"].count() == 0 || (temp.size() == 1 && temp[0] == "all")) {
    return measures;
  }
...
std::tuple<unsigned int, unsigned int> BethYw::parseYearsArg(cxxopts::ParseResult& args) {
  ...
  auto temp = args["y"].as<std::string>();

  if(temp.find("-") == std::string::npos) {
    unsigned int year = atoi(temp.c_str());
    return {year, year};
  }
...
void BethYw::loadAreas(Areas &areas, std::string dir, 
std::unordered_set<std::string> areasFilter) {
    std::string fullpath = dir + "areas.csv";
    InputFile inp(fullpath);

    areas.populate(inp.open(), InputFiles::AREAS.PARSER, InputFiles::AREAS.COLS);
}
...
void BethYw::loadDatasets(Areas& areas,
  ...
  std::tuple<unsigned int, unsigned int> yearsFilter) 
  {
    for(auto dataset : datasetsToImport) {
      std::cout << dataset.CODE << " " << dataset.PARSER << "\n";
      std::string fullpath = dir + dataset.FILE;
      InputFile inp(fullpath);
      areas.populate(inp.open(), dataset.PARSER, dataset.COLS, &areasFilter, &measuresFilter, &yearsFilter);
    }
